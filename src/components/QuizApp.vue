<template>
  <div class="quiz-container max-w-xl mx-auto bg-gray-100 p-6 rounded-lg shadow-md mt-10">
    ...
    <div v-else>
      ...
      <QuizQuestion v-if="currentQuestion < questions.length" :question="questions[currentQuestion]"
        @answer="handleAnswer" :timer="timer" />

      <p v-if="feedback && !quizEnded" class="feedback mt-4 text-lg font-semibold text-center"
...
export default defineComponent({
  ...
  setup() {
    ...
    const questions = ref([
      ...
      { text: 'Qui a peint la Joconde ?', options: ['Vincent van Gogh', 'Pablo Picasso', 'Léonard de Vinci', 'Claude Monet'], answer: 'Léonard de Vinci' },
      { text: 'Quel est le plus petit pays du monde ?', options: ['Vatican', 'Monaco', 'Malte', 'Liechtenstein'], answer: 'Vatican' },
      { text: 'Quel métal est le plus léger ?', options: ['Or', 'Plomb', 'Aluminium', 'Lithium'], answer: 'Lithium' },
      { text: 'Dans quelle ville se trouve la statue du Christ Rédempteur ?', options: ['Rio de Janeiro', 'Lisbonne', 'Mexico', 'Lima'], answer: 'Rio de Janeiro' },
      { text: 'Quel est le plus long fleuve du monde ?', options: ['Nil', 'Amazone', 'Yangtsé', 'Mississippi'], answer: 'Amazone' },
      { text: 'Quel est le symbole chimique de l\'or ?', options: ['Au', 'Ag', 'Fe', 'Hg'], answer: 'Au' },
      { text: 'Quelle planète est la plus proche du Soleil ?', options: ['Mercure', 'Vénus', 'Mars', 'Jupiter'], answer: 'Mercure' },
      { text: 'Quelle est la capitale du Japon ?', options: ['Tokyo', 'Kyoto', 'Osaka', 'Hiroshima'], answer: 'Tokyo' },
    ...
    const startTimer = () => {
      interval.value = setInterval(() => {
        timer.value -= 1;
        if (timer.value === 0) {
          timeUpMessage.value = "Temps écoulé !";
          isCorrect.value = false;
          feedback.value = timeUpMessage.value;
          nextQuestion();
        }
      }, 1000);
    };
